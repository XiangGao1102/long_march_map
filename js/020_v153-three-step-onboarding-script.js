...
(() => {
  ...
  function getMapRect() {
    ...
    const base = (scene || app).getBoundingClientRect();
    const appRect = (app || scene).getBoundingClientRect();
  ...
  function getTargetRect(step) {
    ...
    return {
      ...
      width: Math.min(innerWidth - 12, rect.width + pad * 2),
      height: Math.min(innerHeight - 12, rect.height + pad * 2)
    };
  }
  ...
  function positionGuide() {
    ...
    if (step.target === "#mobile-panel-toggle") {
      ...
      if (left + cardRect.width > innerWidth - 14) left = innerWidth - cardRect.width - 14;
      top = rect.top + rect.height / 2 - cardRect.height / 2;
    } else {
      left = rect.left + rect.width / 2 - cardRect.width / 2;
      top = rect.top + rect.height + 18;
      if (top + cardRect.height > innerHeight - 14) top = rect.top - cardRect.height - 18;
    }
    card.style.left = `${clamp(left, 14, innerWidth - cardRect.width - 14)}px`;
    card.style.top = `${clamp(top, 14, innerHeight - cardRect.height - 14)}px`;
  }
