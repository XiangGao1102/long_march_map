...
(() => {
  ...
  if (isIOS) root.classList.add("lm-ios-safe-viewport");
  ...
  function readPhysicalViewport() {
    ...
    /* V166：iOS Safari 无法可靠进入网页 Fullscreen。
       iOS 非原生全屏时直接以 visualViewport 的“真正可见区域”为画布，
       地址栏/工具栏占掉的区域不再计入作品尺寸。安卓仍保持 V140 的完整屏幕逻辑。 */
    if (likelyMobile) {
      if (isIOS && !isNativeFullscreen()) {
        return { w: vw, h: vh };
      }
  ...
  function refreshStableViewport(force = false) {
    ...
    /* V166：安卓继续使用稳定 screen 尺寸；iOS Safari 必须跟随 visualViewport，
       才能在地址栏展开/收起时始终只使用真正可见区域。 */
    if (likelyMobile) {
      if (isIOS && !isNativeFullscreen()) {
        const changed =
          orientationChanged ||
          Math.abs(now.w - stableViewport.w) >= 2 ||
          Math.abs(now.h - stableViewport.h) >= 2;
        if (force || changed) stableViewport = now;
  ...
  compatStyle.textContent = `
    ...
    /* V166 — iOS Safari 可见视口安全画布。
       #app 本身成为 fixed 子元素的 containing block，因此顶栏、底栏、左右栏
       都自动限制在 Safari 地址栏之外，无需逐个元素改位置。 */
    html.lm-ios-safe-viewport:not(.force-css-landscape) #app {
      position: fixed !important;
      left: var(--lm-ios-vv-left, 0px) !important;
      top: var(--lm-ios-vv-top, 0px) !important;
      right: auto !important;
      bottom: auto !important;
      width: var(--lm-ios-vv-width, 100vw) !important;
      height: var(--lm-ios-vv-height, 100vh) !important;
      min-width: 0 !important;
      min-height: 0 !important;
    ...
    html.lm-ios-safe-viewport.force-css-landscape #app {
      left: var(--lm-ios-vv-center-x, 50%) !important;
      top: var(--lm-ios-vv-center-y, 50%) !important;
    }

    html.lm-ios-safe-viewport #scene,
    html.lm-ios-safe-viewport #scene canvas,
    html.lm-ios-safe-viewport #labels-layer,
    html.lm-ios-safe-viewport #terrain-labels {
      width: 100% !important;
      height: 100% !important;
    }
  ...
  function syncLandscapeViewport(force = false) {
    ...
    if (isIOS) {
      ...
      root.style.setProperty("--lm-ios-vv-left", `${left}px`);
      root.style.setProperty("--lm-ios-vv-top", `${top}px`);
      root.style.setProperty("--lm-ios-vv-width", `${width}px`);
      root.style.setProperty("--lm-ios-vv-height", `${height}px`);
      root.style.setProperty("--lm-ios-vv-center-x", `${left + width / 2}px`);
      root.style.setProperty("--lm-ios-vv-center-y", `${top + height / 2}px`);
    }
