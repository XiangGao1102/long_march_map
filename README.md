...
- iOS 逻辑横屏下，不再使用旋转后的 `getBoundingClientRect()` 计算歌词位置，而改用内部未旋转布局坐标 `offsetTop`。
- iOS 不再依赖 Safari 对“旋转祖先 + overflow 容器”的原生 smooth scroll，改用 `requestAnimationFrame` 做等效平滑滚动。
- 每次进入下一句歌词时，当前歌词仍自动滚动到右侧歌词区域中央。
- Android、PC 继续使用原来的滚动实现，完全不改。
- 左侧地图、右侧布局、音乐、节点、3D主界面及其他功能均未改变。
