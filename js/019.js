...
(() => {
  ...
  ready(() => {
    ...
    const open = () => {
      ...
      if (mapImage && !mapImage.getAttribute("src")) {
        ...
        mapImage.src =
          (isIOSImmersive && mapImage.dataset.srcIos)
            ? mapImage.dataset.srcIos
            : mapImage.dataset.src;

        /* V170：iOS大图解码完成后再测一次地图容器。
           避免Safari在旋转层刚显示时首次clientWidth/clientHeight尚未稳定，
           导致canvas尺寸为0或错误而出现黑屏。 */
        const remeasureAfterImageReady = () => {
          if (!state.open) return;
          requestAnimationFrame(() => {
            measureMap();
            requestAnimationFrame(() => measureMap());
          });
        };
        mapImage.addEventListener("load", remeasureAfterImageReady, { once: true });
        mapImage.decode?.().then(remeasureAfterImageReady).catch(() => {});
      }
    ...
    const close = () => {
      ...
      const lyricScrollRaf = __imLyricScrollRaf__.get(lyricScroller);
      if (lyricScrollRaf) {
        cancelAnimationFrame(lyricScrollRaf);
        __imLyricScrollRaf__.delete(lyricScroller);
      }
        ...
        <section class="im-map-column" aria-label="长征节点地图">
          <div class="im-map-viewport">
            <div class="im-map-canvas">
              <img class="im-map-image"
                   data-src="assets/images/img_aaff512d2e35fde3.webp"
                   data-src-ios="assets/images/img_aaff512d2e35fde3_ios.webp"
                   decoding="async"
                   alt="中央红军长征十六节点艺术地图">
  ...
  const __imLyricScrollRaf__ = new WeakMap();

  function __imOffsetTopInsideScroller__(item, scroller) {
    let top = 0;
    let node = item;
    let guard = 0;

    /* V171：不要用 getBoundingClientRect() 计算iOS歌词位置。
       沉浸式外层在iOS中整体 rotate(90deg)，Safari返回的是旋转后的屏幕坐标，
       与内部 scrollTop 的逻辑纵轴并不一致。offsetTop 链则始终位于未旋转的
       内部布局坐标系，与Android横屏的歌词滚动逻辑完全一致。 */
    while (node && node !== scroller && guard < 20) {
    ...
    /* 极少数Safari布局下offsetParent链不会直接落到scroller。
       退化为offsetTop本身，仍优先保持逻辑布局坐标而不读取旋转后的屏幕矩形。 */
    return Number(item.offsetTop || 0);
  }

  function __imAnimateIOSLyricScroll__(scroller, target, instant = false) {
    const oldRaf = __imLyricScrollRaf__.get(scroller);
    if (oldRaf) cancelAnimationFrame(oldRaf);

    const maxTop = Math.max(0, scroller.scrollHeight - scroller.clientHeight);
    const end = Math.max(0, Math.min(maxTop, target));

    if (instant) {
      scroller.scrollTop = end;
      __imLyricScrollRaf__.delete(scroller);
      return;
    }
    ...
    const step = (now) => {
      ...
      /* smoothstep：与Android原生smooth滚动的观感接近，但不依赖Safari
         对“旋转祖先 + overflow scroller”的原生smooth scroll实现。 */
      const eased = t * t * (3 - 2 * t);
      scroller.scrollTop = start + distance * eased;

      if (t < 1) {
        const raf = requestAnimationFrame(step);
        __imLyricScrollRaf__.set(scroller, raf);
      } else {
        scroller.scrollTop = end;
        __imLyricScrollRaf__.delete(scroller);
      }
  ...
  function scrollLyricToCenter(item, scroller, instant = false) {
    ...
    if (isIOS && document.documentElement.classList.contains("force-css-landscape")) {
      const itemTop = __imOffsetTopInsideScroller__(item, scroller);
      const itemHeight = item.offsetHeight || 0;
      const target = itemTop - (scroller.clientHeight - itemHeight) / 2;
      __imAnimateIOSLyricScroll__(scroller, target, instant);
      return;
    }

    /* Android / PC 完全保留原来的滚动方式。 */
